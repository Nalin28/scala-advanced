...
object FBoundedPolymorphism extends App{
  ...
  // in case of type hierarchy how do I enforce a super type over a current type
...
//  trait Animal[A <: Animal[A]]{ self: A => // whatever descendants I implement it must be an A as well #1
//    def breed: List[Animal[A]]
//  }
//
//  class Cat extends Animal[Cat]{
...
//    override def breed: List[Animal[Fish]] = List(new Cod) // wrong -> sharks do'nt breed to cod
//  }
//
...
//  // cat.breed -> compiler will catch this!!
  ...
  class Dog
  object Dog{
    implicit object DogsCanBreed extends Animal[Dog] {
      override def breed(a: Dog): List[Dog] = List()
    }
  }
