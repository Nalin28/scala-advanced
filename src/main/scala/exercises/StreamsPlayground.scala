...
object EmptyStream extends MyStream[Nothing]{
  ...
  def take(n: Int): MyStream[Nothing] = this //takes first n elements out of this stream
...
object StreamsPlayground extends App{
  ...
  println(startFrom0.flatMap(x => new Cons(x+1,new Cons(x+1,EmptyStream))).take(10).toList())
  ...
  var count: Int = 2
  def erato(numbers: MyStream[Int]): MyStream[Int] =
    if(numbers.isEmpty) numbers
    else new Cons(numbers.head,erato(numbers.tail.filter(_ %numbers.head != 0)))

  println(erato(MyStream.from(2)(_ + 1)).take(10).toList())
}
